...
import { TEMP_OTP_CODE, createTempAccount } from "@/lib/temp-auth";
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const account = createTempAccount(email, password);
  ...
  if (showOtp) {
    return (
      <AuthLayout icon={Mail} title="Verify your email" subtitle="Enter the verification code to continue">
        ...
        <Button className="w-full h-12 font-medium" onClick={handleVerify} disabled={otpCode.length < 6}>
          Verify
        </Button>
